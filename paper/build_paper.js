...
col.push(body("On the signal side, temperature scaling [9] remains the standard correction for neural overconfidence, and semantic entropy [10] improves uncertainty estimation by clustering sampled generations by meaning at the cost of repeated inference. Table I positions this work against the closest systems. Our study is distinguished on three axes simultaneously: energy is measured on hardware rather than estimated, discarded computation is charged to the query that caused it, and confidence routing is extended to a visual workload."));
col.push(...tbl("TABLE I. COMPARISON WITH CLOSEST SYSTEMS",
  ["System", "Carbon-aware", "Energy source", "Training-free", "Discarded run charged"],
  [["FrugalGPT [3]", "No", "None", "No", "No"],
   ["AutoMix [4]", "No", "None", "Yes", "No"],
   ["C3PO [5]", "No", "None", "Yes", "No"],
   ["GreenServ [6]", "Partial", "Estimated", "No", "No"],
   ["GAR [7]", "Yes", "Estimated", "No", "No"],
   ["EcoLogits [8]", "Estimate only", "Estimated", "Yes", "n/a"],
   ["This work", "Yes", "Measured", "Yes", "Yes"]],
  [1180, 900, 880, 780, 1020]));
...
col.push(...fig(`${FIGS}/fig_calibration.png`, 240, 170,
  "Fig. 1. Expected Calibration Error before and after temperature scaling for the seven models calibrated in this study. Every model is overconfident before correction; the 4-bit large model requires the largest temperature."));
...
col.push(body("At the operating point maximising quality per gram on ShareGPT, full accounting reports a 47.4 percent carbon reduction where the naive convention reports 54.2 percent, an overstatement of 9.7 percent in relative terms. The divergence grows with escalation. Fig. 2 shows both curves against the always-large baseline: the naive curve lies systematically to the left of the truth, and beyond roughly 75 percent escalation the full-accounting curve crosses to the right of the baseline, meaning the cascade emits more than never cascading while the naive convention still reports a saving of 15.2 percent."));
col.push(...fig(`${FIGS}/fig_accounting_pareto.png`, 240, 184,
  "Fig. 2. Quality-carbon frontier on ShareGPT under both accounting conventions. Naive accounting understates carbon at every operating point, and at high escalation reports savings where full accounting shows an increase over always-large."));
...
col.push(...tbl("TABLE II. SHAREGPT, 500 QUERIES, FULL ACCOUNTING",
